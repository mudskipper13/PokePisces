...
enum {
    ...
    FIELD_MOVE_CUT,         // FLAG_BADGE03_GET
    FIELD_MOVE_FLY,         // FLAG_BADGE04_GET
    FIELD_MOVE_STRENGTH,    // FLAG_BADGE05_GET
    FIELD_MOVE_SURF,        // FLAG_BADGE06_GET
...
static void Task_Mints(u8 taskId)
{
    ...
    u32 oldPersonality = GetMonData(&gPlayerParty[tMonId], MON_DATA_PERSONALITY, 0);
    bool8 wasShiny = IsShinyOtIdPersonality(otId, oldPersonality);
    ...
    case 5:
        if(wasShiny) {
            ...
            while (tNewNature != GetNatureFromPersonality(personality));                
        } else {
            do {
                personality = Random32();
            }
        ...
        UpdateMonPersonality(&gPlayerParty[tMonId].box, personality);
...
void ItemUseCB_Mints(u8 taskId, TaskFunc task)
{
    ...
    tMonId = gPartyMenu.slotId;
    tSpecies = GetMonData(&gPlayerParty[tMonId], MON_DATA_SPECIES, NULL);
    tCurrNature = GetMonData(&gPlayerParty[tMonId], MON_DATA_PERSONALITY, 0) % NUM_NATURES;
