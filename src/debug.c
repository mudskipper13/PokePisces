    ...
    switch (id)
    {
        ...
        case DEBUG_FLAGVAR_MENU_ITEM_TOGGLE_LOCATIONS:
            result = FlagGet(FLAG_VISITED_LITTLEROOT_TOWN) &&
                ...
                FlagGet(FLAG_VISITED_ZOTPYRE) &&
                FlagGet(FLAG_VISITED_OZONE_INSTITUTE) &&
...
static void DebugAction_Util_Fly(u8 taskId)
{
    ...
    FlagSet(FLAG_VISITED_ZOTPYRE);
    FlagSet(FLAG_VISITED_OZONE_INSTITUTE);
...
static void DebugAction_FlagsVars_ToggleFlyFlags(u8 taskId)
{
    ...
    if (FlagGet(FLAG_LANDMARK_BATTLE_FRONTIER))
    {
        ...
        FlagClear(FLAG_VISITED_ZOTPYRE);
        FlagClear(FLAG_VISITED_OZONE_INSTITUTE);
...
#define SOUND_LIST_BGM \
    ...
    X(MUS_MT_FREEZE) \
    X(MUS_RANGER_RINGTOWN) \
    X(MUS_SUNKEN_KURU_RUINS) \
    X(MUS_RANGER_TUTORIAL_CAPTURE) \
    X(MUS_SINISTER_WOODS) \
    X(MUS_SILENT_CHASM) \
    X(MUS_LAPIS_CAVE) \
    X(MUS_MT_BLAZE) \
    X(MUS_MT_BLAZE_PEAK) \
