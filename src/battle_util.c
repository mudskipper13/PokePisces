...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_INGRAIN: // ingrain
            if ((gStatuses3[battler] & STATUS3_ROOTED) && !BATTLER_MAX_HP(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && gBattleMons[battler].hp != 0 && GetBattlerAbility(battler) != ABILITY_STRONGHOLD)
            {
        ...
        case ENDTURN_AQUA_RING: // aqua ring
            if ((gStatuses3[battler] & STATUS3_AQUA_RING) && !BATTLER_MAX_HP(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && gBattleMons[battler].hp != 0 && GetBattlerAbility(battler) != ABILITY_STRONGHOLD)
            {
        ...
        case ENDTURN_HEARTHWARM: // aqua ring
            if ((gStatuses4[battler] & STATUS4_HEARTHWARM) && !BATTLER_MAX_HP(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && gBattleMons[battler].hp != 0 && GetBattlerAbility(battler) != ABILITY_STRONGHOLD) 
            {
                gBattleMoveDamage = GetDrainedBigRootHp(battler, gBattleMons[battler].maxHP / 16);
        ...
        case ENDTURN_POISON: // poison
            if ((gBattleMons[battler].status1 & STATUS1_POISON) && gBattleMons[battler].hp != 0)
            {
                ...
                {
                    if (!BATTLER_MAX_HP(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && GetBattlerAbility(battler) != ABILITY_STRONGHOLD)
                    {
                        if (IsSpeciesOneOf(gBattleMons[battler].species, gMegaBosses))
                            gBattleMoveDamage = gBattleMons[battler].maxHP / 16;
                        else
    ...
    case ABILITYEFFECT_ENDTURN: // 1
        if (gBattleMons[battler].hp != 0)
        {
            ...
            case ABILITY_SOLAR_POWER:
                if (IsBattlerWeatherAffected(battler, B_WEATHER_SUN)
                && (gDisableStructs[gBattlerAttacker].daybreakCounter < 3))
                {
                ...
                if (gDisableStructs[battler].isFirstTurn != 2 && gDisableStructs[gBattlerAttacker].daybreakCounter != 0)
                {
                    gBattlerAttacker = battler;
                    BattleScriptPushCursorAndCallback(BattleScript_SolarPowerActivatesEveryone);
                    effect++;
...
u32 IsAbilityPreventingEscape(u32 battler)
{
    ...
    if (IS_BATTLER_OF_TYPE(battler, TYPE_GHOST))
        return 0;
    if ((id = IsAbilityOnOpposingSide(battler, ABILITY_SPIRALYSIS)) && GetBattlerAbility(battler) != ABILITY_SPIRALYSIS && (!(IS_BATTLER_OF_TYPE(battler, TYPE_PSYCHIC))) && IsBattlerGrounded(battler) && gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN)
        return id;
...
static u8 DamagedNanabBerryEffect(u32 battler, u32 itemId, u32 statId, bool32 end2)
{
    ...
    && gDisableStructs[battler].stockpileCounter < 3)
    {
        ...
        if (end2)
        {
            BattleScriptExecute(BattleScript_NanabBerryEnd);
        }
        else
        {
            BattleScriptPushCursor();
...
u8 ItemBattleEffects(u8 caseID, u32 battler, bool32 moveTurn)
{
    ...
    case ITEMEFFECT_NORMAL:
        if (gBattleMons[battler].hp)
        {
            ...
            LEFTOVERS:
#if B_HEAL_BLOCKING >= GEN_5
                if (gBattleMons[battler].hp < gBattleMons[battler].maxHP && !moveTurn && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && GetBattlerAbility(battler) != ABILITY_STRONGHOLD)
#else
...
#if B_HEAL_BLOCKING >= GEN_5
                if (gBattleMons[battler].hp < (gBattleMons[battler].maxHP / 2) && !moveTurn && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && GetBattlerAbility(battler) != ABILITY_STRONGHOLD)
#else
            ...
            case HOLD_EFFECT_YELLOW_SODA:
                if (gBattleMons[battler].species == SPECIES_VOREON && gBattleMons[battler].hp < (gBattleMons[battler].maxHP / 2) && !moveTurn && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && GetBattlerAbility(battler) != ABILITY_STRONGHOLD)
                {
                    gBattleMoveDamage = gBattleMons[battler].maxHP / 2;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
    ...
    case ITEMEFFECT_KINGSROCK:
        ...
        case HOLD_EFFECT_FLINCH:
        {
            ...
            if (ability == ABILITY_SERENE_GRACE || ability == ABILITY_RISKTAKER || IsAbilityOnSide(battler, ABILITY_SERENE_AURA))
...
static inline u32 CalcMoveBasePower(u32 move, u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_FIERY_WRATH:
        basePower = 90 + (10 * gBattleStruct->timesGotHit[GetBattlerSide(battlerAtk)][gBattlerPartyIndexes[battlerAtk]]);
        basePower = (basePower > 200) ? 200 : basePower;
        break;
...
#define TYPE_POWER_INCREASE_CHECK(typeCheck) {    \
    if (moveType == typeCheck) { \
        holdEffectModifier = uq4_12_add(UQ_4_12(1.0), sPercentToModifier[20]); \
...
#define TERRAIN_TYPE_BOOST UQ_4_12(1.5)
    ...
    if (gSpecialStatuses[battlerAtk].gemBoost)
        modifier = uq4_12_multiply(modifier, uq4_12_add(UQ_4_12(1.0), sPercentToModifier[gSpecialStatuses[battlerAtk].gemParam]));
    ...
    holdEffectModifier = uq4_12_add(UQ_4_12(1.0), sPercentToModifier[holdEffectParamAtk]);
    ...
    */
    ...
    case HOLD_EFFECT_CLEANSE_TAG:
        if ((CountBattlerStatDecreases(battlerDef, TRUE)) > 0)
            modifier = uq4_12_multiply_half_down(modifier, uq4_12_add(UQ_4_12(1.0), sPercentToModifier[CountBattlerStatDecreases(battlerDef, TRUE) * 15]));
        break;
...
static inline uq4_12_t GetFrenzyAttackerModifier(u32 battlerAtk, bool32 isCrit)
{
    if (gDisableStructs[battlerAtk].frenzyCounter != 0 && isCrit)
        return uq4_12_add(UQ_4_12(1.0), sPercentToModifier[gDisableStructs[battlerAtk].frenzyCounter * 30]);
    return UQ_4_12(1.0);
}
...
static inline uq4_12_t GetFrenzyDefenderModifier(u32 battlerDef)
{
    if (gDisableStructs[battlerDef].frenzyCounter != 0)
        return uq4_12_add(UQ_4_12(1.0), sPercentToModifier[gDisableStructs[battlerDef].frenzyCounter * 20]);
    return UQ_4_12(1.0);
}
                                       ...
                                       u32 holdEffectAtk, u32 holdEffectDef, u32 abilityAtk, u32 abilityDef)
{
    ...
    {
        dmg = dmg + (gBattleMons[battlerDef].maxHP * 15 / 100);
    }
    ...
    {
        dmg = dmg + (gBattleMons[battlerDef].maxHP / 5);
    }
